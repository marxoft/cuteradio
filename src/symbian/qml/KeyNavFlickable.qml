...
Flickable {
    ...
    QtObject {
        ...
        function itemGetsFocus(item) {
            return (item.hasOwnProperty("checked")) || (item.hasOwnProperty("cursorPosition")) || (item.hasOwnProperty("subItemIndicator")) || (item.hasOwnProperty("item")) || (item.hasOwnProperty("model"));
        }
    ...
    Connections {
        ...
        onStatusChanged: if ((progressDialog.status === DialogStatus.Closed) && (visible)) forceActiveFocus();
    }
